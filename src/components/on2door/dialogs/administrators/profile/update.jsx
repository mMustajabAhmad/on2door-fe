...
// API Imports
import { getAdminApi, updateAdminApi } from '@/app/api/on2door/actions'
...
const EditProfileDialog = ({ open, setOpen, currentUser }) => {
  ...
  const { data: userData } = useQuery({
    queryKey: ['administrator', currentUser?.id],
    queryFn: () => getAdminApi(currentUser?.id),
  ...
  const onSubmit = data => {
    ...
    updateUser({ id: currentUser?.id, payload })
  }
  ...
  return (
    <Dialog fullWidth open={open} onClose={handleClose} maxWidth='md' scroll='body' closeAfterTransition={false}>
      <DialogTitle className='flex gap-2 flex-col items-center sm:pbs-16 sm:pbe-6 sm:pli-16'>
        <div className='max-sm:is-[80%] max-sm:text-center text-xl font-semibold'>Edit Profile Information</div>
