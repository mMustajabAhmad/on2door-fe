...
// API Imports
import { createHubApi, getTeamsApi } from '@/app/api/on2door/actions'
...
const CreateHubDialog = ({ open, setOpen }) => {
  ...
  const { mutate: createHub, isPending } = useMutation({
  ...
  return (
    <Dialog fullWidth open={open} onClose={handleClose} maxWidth='md' scroll='body' closeAfterTransition={false}>
      <DialogTitle className='flex gap-2 flex-col items-center sm:pbs-16 sm:pbe-6 sm:pli-16'>
        <div className='max-sm:is-[80%] max-sm:text-center text-xl font-semibold'>Create New Hub</div>
