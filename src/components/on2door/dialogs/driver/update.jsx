...
// API Imports
import { getDriverApi, updateDriverApi, getTeamsApi } from '@/app/api/on2door/actions'
...
const EditDriverDialog = ({ open, setOpen, currentDriver }) => {
  ...
  const { data: driverData } = useQuery({
    queryKey: ['driver', currentDriver?.id],
    queryFn: () => getDriverApi(currentDriver?.id),
    enabled: !!currentDriver?.id && open
  })
...
return (
  ...
  useEffect(() => {
    if (driverData) {
      ...
      reset({
        ...
        team_ids: driver.team_ids?.map(id => id.toString()) || []
      })
    }
  }, [driverData, reset])
  ...
  const onSubmit = data => {
    ...
    updateDriver({ id: currentDriver?.id, payload })
  }
  ...
  return (
    <Dialog fullWidth open={open} onClose={handleClose} maxWidth='md' scroll='body' closeAfterTransition={false}>
      <DialogTitle variant='h4' className='flex gap-2 flex-col items-center sm:pbs-16 sm:pbe-6 sm:pli-16'>
        <div className='max-sm:is-[80%] max-sm:text-center'>Edit Driver Information</div>
