...
const DriverListTable = ({
  ...
  const transformApiData = apiData => {
    if (!apiData?.drivers?.data) return []

    return apiData.drivers.data.map(driver => ({
      id: driver.id,
      fullName: `${driver.attributes.first_name} ${driver.attributes.last_name}`,
      email: driver.attributes.email,
      phone_number: driver.attributes.phone_number,
      organization_id: driver.attributes.organization_id,
      is_active: driver.attributes.is_active,
      status: driver.attributes.is_active ? 'active' : 'inactive'
    }))
  }
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            ...
            />
            <IconButton>
              <Link href={getLocalizedUrl(`/drivers/${row.original.id}`, locale)} className='flex'>
            ...
            <OpenDialogOnElementClick
              ...
              dialogProps={{ data: data, currentDriver: row.original }}
            />
          </div>
        ),
        enableSorting: false
  ...
  return (
    <>
      ...
      <CreateDriverDialog open={addUserOpen} setOpen={setAddUserOpen} />
    </>
  )
}
